...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dataset', type=str, default="dataset1")
    parser.add_argument('-v', '--version', type=str, default='my_icp', help='open3d or my_icp')
    parser.add_argument('--data_root', type=str, default='data_collection_dataset1/first_floor/')
    args = parser.parse_args()

    if args.floor == 1:
        if args.dataset == "dataset1":
            args.data_root = "data_collection_dataset1/first_floor/"
        elif args.dataset == "dataset2":
            args.data_root = "data_collection_dataset2/first_floor/"
    elif args.floor == 2:
        if args.dataset == "dataset1":
            args.data_root = "data_collection_dataset1/second_floor/"
        elif args.dataset == "dataset2":
            args.data_root = "data_collection_dataset2/second_floor/"

    #data list 

    rgb_file_list = glob.glob(r'./'+str(args.data_root)+'sem/*')
    #rgb_file_list = glob.glob(r'./'+str(args.data_root)+'gt/*') #choose the gt or  sem
    depth_file_list = glob.glob(r'./'+str(args.data_root)+'depth/*')

    rgb_file_list = ["./"+str(args.data_root)+"sem/{}.png".format(i+1) for i in range(len(rgb_file_list))]
    #rgb_file_list = ["./"+str(args.data_root)+"gt/{}.png".format(i+1) for i in range(len(rgb_file_list))]
    ...
    o3d.visualization.draw_geometries([hiddenceiling_pointcloud])
